...
def sorte(evs, N_max, T):
    M = len(evs)
    delta_evs = np.flipud(np.diff(evs))
    var_list = []
    
    for i in range(len(delta_evs)-1):
        var = np.sum((delta_evs[i:] - np.sum(delta_evs[i:])/(M-i-1))**2) / (M-i-1)
        var_list.append(var)

    sorte_list = []
    
    for i in range(len(var_list)-1):
        sorte =  var_list[i+1]/var_list[i]
        sorte_list.append(sorte)

    N = np.argmin(sorte_list)+1
    N = min(N, N_max)
