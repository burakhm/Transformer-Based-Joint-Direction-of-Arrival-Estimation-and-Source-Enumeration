...
def eigen_threshold(evs, T):
    M = len(evs)
    t = 1.5
    N = M-1
    
    for i in range(len(evs)-1):
        avg_noise = np.sum(evs[i+1:]) / (M-i-1)
        upper_threshold = ((i+2) * (1+t*(T*(i+2))**-0.5) / (1-t*(T*(i+1))**-0.5) - (i+1)) * avg_noise
        if evs[i] > upper_threshold:
            N = i+1
    return N
...
def music(R,M,N_max,T,sensor_pos,res):
    ...
    D = D[idx]
    V = V[:,idx]
    N = eigen_threshold(np.flipud(D), T)
